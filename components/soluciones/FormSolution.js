...
const FormModule = ({ setShowForm, dataSolution }) => {
    const SolucionContext = useContext(solucionContext)
    const { addSolution, updateSolution } = SolucionContext
    ...
    const formik = useFormik({
        initialValues: {
            name: dataSolution ? dataSolution.name : '',
            description: dataSolution ? dataSolution.description : '',
            status: dataSolution ? dataSolution.status : '',
        },
        ...
        onSubmit: async data => {
            ...
            if (dataSolution) await updateSolution(data, dataSolution)
            else await addSolution(data)
            ...
            >
                ...
                <Row>
                    <Button
                        node="button"
                        type="submit"
                        className={`btn waves-effect waves-light ${dataSolution ? 'blue' : 'green'}`}
                        icon={<Icon right>send</Icon>}
                    ><b>{dataSolution ? 'Editar' : 'Crear'}</b></Button>
                </Row>
            </form>
        </div>
    )
